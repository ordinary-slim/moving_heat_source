...
def deactivateLeftOfStartPoint(p):
    nels = p.domain.mesh.nels
    activeEls = []
    for ielem in range( nels ):
        e = p.domain.mesh.getElement( ielem )
        if (e.getCentroid()[0] < startPoint):
            activeEls.append( ielem )
    p.domain.setActivation( mhs.MeshTag( p.domain.mesh, p.domain.mesh.dim, activeEls ) )
...
def runCoupled(caseName="coupled", plot=False):
    ...
    if fixedProblemInput["print"]:
        deactivateLeftOfStartPoint( pFixed )

    adimFineDt = fixedProblemInput["adimDtCoupled"]
    adimMaxDt  = fixedProblemInput["adimDtCoupled"]
